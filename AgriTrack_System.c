...
// Data Structure: Singly Linked List (Stores crops)
struct crop {
    char name[50];
    struct crop *next; 
};

// Data Structure: Queue_FIFO(Stores harvested items in inventory)
struct item {
char name[50];
int qty;          
int days;          
...
// Data Structure: Stack_LIFO (Stores dispatched items for undo operations)
struct undo {
char name[50];
int qty;
int days;
...
// pointers for Linked List, Queue, and Stack
...
int found = 0;
 ...
 if (head == NULL) {
   printf("Error: No crops registered yet. Please add a crop first (Option 1).\n");
    ...
    // Using Linear Search to verify if crop exists
    while (temp != NULL) {
...
// ALGORITHM 2: Bubble Sort (Sort Inventory by Expiry Days)
void sortItems() {
struct item *i, *j;
char n[50];
int q, d;
...
// Push to Stack (Save Dispatch for Undo)
...
// ALGORITHM 1: Linear Search (Find Item in Inventory)
void searchItem() {
    struct item *temp = front;
    char find[50];
    int found = 0;
...
// Condition Check (Spoilage Warning)
void spoilage() {
struct item *temp = front;
int found = 0;
 while(temp != NULL) {
...
int main() {
    ...
    do {
        ...
        printf("5. Sort by Expiry\n");   
        printf("6. Dispatch\n");         
        printf("7. Undo Dispatch\n");   
        printf("8. Search Crop\n");      
